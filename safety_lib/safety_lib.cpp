...
namespace devtitans::safety {                       // Entra no pacote devtitans::smartlamp

int Safety::connect() {
    char dirPath[] = "/sys/kernel/safety";
    struct stat dirStat;
    ...
    // bool allowSimulated = GetBoolProperty("devtitans.safety.allow_simulated", true);
    // if (!allowSimulated)
...
int Safety::readFileValue(string file) {
    ...
    if (connected == 1) {                          // Conectado. Vamos solicitar o valor ao dispositivo
        int value;
        string filename = string("/sys/kernel/safety/") + file;
...
bool Safety::writeFileValue(string file, int value) {
    int connected = this->connect();
...
int Safety::getPresence() {
    return this->readFileValue("presence");
}
